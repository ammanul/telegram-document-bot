...
from worker.app.services.db import Base, engine
...
def init_db() -> None:
    ...
    # Keep DB schema in sync with the server by removing the unique
    # (job_id, telegram_file_id) constraint if it exists.
    from sqlalchemy import text as _text  # type: ignore

    with engine.connect() as conn:
        try:
            conn.execute(_text("ALTER TABLE job_files DROP CONSTRAINT IF EXISTS uq_jobfile_job_file"))
            conn.commit()
        except Exception:
            pass
